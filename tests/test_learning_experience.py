...
def test_course_catalog_owns_every_teaching_lesson_once() -> None:
    ...
    assert catalog["curriculum_version"] == 9
    for course in courses:
...
def test_foundations_has_versioned_course_sequence() -> None:
    ...
    assert completion["curriculum_version"] == 9
    ...
    assert "curriculum_version: 9" in course_ui
    assert "state.curriculum_version = 9" in course_ui
